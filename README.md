...
* [KLUE/BERT-base](https://huggingface.co/klue/bert-base) 사용
  
    `python run_graph.py`
    
        * model = Baseline : 기존 베이스라인
        * model = LMForSequenceClassification : 제안 모델
        * model = LMForSequenceClassification_only_LabelAttn : LAN만 사용한 경우, 구문정보 학습 제거
        * model = LMForSequenceClassification_without_LabelAttn : 구문 정보만 학습한 경우, LAN 학습 제거
